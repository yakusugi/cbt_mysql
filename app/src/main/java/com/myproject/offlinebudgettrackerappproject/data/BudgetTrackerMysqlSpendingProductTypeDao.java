...
public class BudgetTrackerMysqlSpendingProductTypeDao extends BaseSpendingDao {
    ...
    public void getSearchProductTypeList(BudgetTrackerMysqlSpendingDto dto, MysqlSpendingListCallback callback) {
        try {
            String serverUrl = loadServerConfig("server_url");
            String phpFile = loadServerConfig("spending_product_type_search_php_file");
            String endpoint = serverUrl + phpFile;

            Map<String, String> params = new HashMap<>();
            params.put("email", SharedPreferencesManager.getUserEmail(context));
            params.put("product_type", dto.getProductType());
